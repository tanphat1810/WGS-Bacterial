...
Workflow gốc bạn cung cấp là một pipeline WGS cho **Bacillus** từ dữ liệu Illumina paired-end, đi qua các khối chính: QC dữ liệu thô, làm sạch reads, QC sau trimming, lắp ráp de novo bằng SPAdes, đánh giá bằng QUAST, tìm 16S rRNA bằng Barrnap rồi trích FASTA bằng BEDTools, so sánh ANI bằng FastANI, thử assembly thay thế bằng Unicycler, scaffolding theo reference bằng RagTag, và cuối cùng chuẩn bị annotation bằng Bakta. Bản viết lại dưới đây chuẩn hóa logic của workflow đó, chuyển toàn bộ đường dẫn sang dạng đại diện, và làm rõ bước nào là **kết thúc khi chạy thành công** và bước nào chỉ là **nhánh điều kiện / nhánh cứu hộ**. fileciteturn0file0

Điểm quan trọng nhất cần ghi rõ trong README là: **Bakta là bước cuối cùng của một lần chạy thành công**, nhưng chỉ sau khi đã **chọn được assembly cuối cùng**. Trong đường chạy chuẩn, assembly cuối cùng thường là output của **SPAdes** nếu QUAST và các kiểm tra liên quan cho thấy assembly đạt yêu cầu. **Unicycler** không phải bước bắt buộc sau SPAdes; nó là một **nhánh thay thế để so sánh hoặc cứu hộ** khi assembly SPAdes còn phân mảnh hoặc có dấu hiệu chưa tốt. **RagTag** cũng không phải bước bắt buộc; nó là **nhánh reference-guided scaffolding có điều kiện**, chỉ nên dùng khi đã có **reference đủ gần** và có nhu cầu sắp xếp/orient contig cho dễ đọc hoặc để cải thiện tính liên tục. **BEDTools trích 16S** chỉ chạy khi Barrnap thật sự tìm thấy feature 16S trong GFF. citeturn11search0turn18search0turn11search1turn13view0turn17search3turn3search0turn7view0

Mục tiêu của README này là để người đọc hiểu nhanh ba điều: pipeline này dùng để làm gì, cần chuẩn bị gì trước khi chạy, và ra quyết định như thế nào giữa đường chạy **thành công ngay** với đường chạy **điều kiện / khắc phục sự cố**. Kết quả kỳ vọng trong ca chạy thành công là: QC trước/sau trimming, clean reads, assembly đã được đánh giá, rRNA/16S nếu có, ANI so với reference gần, và bộ annotation Bakta cho **assembly cuối cùng được chọn**. citeturn0search0turn17search2turn9search0turn18search0turn11search1turn13view0turn16search2turn11search0
